 ...
 * La contraseña sera un String simple, pero al momento de guardar se encripta
 * con md5
 * 
 * @author fpezzola
 *
...
public class LoginService {
	...
	/**
	 * Realiza el login. Valida el usuario existente y hashea la contraseña con
	 * md5 para pdoer comparar con el archivo de usuarios. Si el usuario es
	 * veridico y su contraseña tambien, lo mete en el threadlocal para luego
	...
	public static String login(String usr, String password) throws Exception { 
		...
		if (!pswArchivo.isEmpty()) {//si existe un usuario:
			...
			if (!encriptMD5.equals(pswArchivo)) //comparo la pass con la del archivo
				errors = "La contraseña es incorrecta"; // no coincide la contrase�a			 
			 else 
				UserProvider.setuUser(usr); //coincide la contrase�a
		} else 
	...
	private static String darContraseñaUsuario(String usr) throws Exception {
		...
		try {
			...
			while ((line = br.readLine()) != null) { //para leer hasta el final del archivo
				...
				pswArchivo = line.substring(index + 1, line.length()); //Obtengo la contrase�a del archivo
				if (usr.equals(usrArchivo)) // compara a ver si coincide con el
				{							// nombre de usuario del archivo
					br.close(); //Cierro el BufferedReader
					return pswArchivo; //Devuelvo la contrase�a
				}
			}
			br.close();//Cierro el BufferedReader
		...
		return ""; // El archivo esta vacio o no se encontro la contrase�a
	}
